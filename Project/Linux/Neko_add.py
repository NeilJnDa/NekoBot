...
def send_email_auto(info):
    email_conf = get_yaml('/home/Neko/email.yml')
...
def send_result_to_admin(accounts):
    email = get_yaml('/home/Neko/email.yml')
    sender_mail = email['sender_mail']
    sender_pass = email['sender_pass']
    to = '15651991790@163.com'
    ...
    for each in accounts:
        print(each)
        content += '<p>' + each['name'] + ':' + each['status'].value + '</p>'
    content += '</body></html>'
    ...
    try:
        ...
        sftp_obj.quit()
        print('sendemail to admin successful!')
...
def send_email_by_qq(email, info):
    ...
    # <p>""" + time + """</p>
    content = '</body></html>'
    content += email['content1'] + '<h3>今日填报结果 :'
    try:
        status = info['status'].value
    except:
        status = '无需显示'
    finally:
        content += info['id'] + '——' + status + '</h3>' + email['content2']
