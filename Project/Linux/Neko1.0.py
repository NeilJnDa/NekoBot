...
def screen_shot(driver, user):
    try:
        picture_url = driver.get_screenshot_as_file('/home/Neko/Log/' + picture_time + '/' + user['name'] + '.png')
...
# 时间
picture_time = time.strftime("%m-%d_%H.%M", time.localtime(time.time()))
# Log与截图保存路径
path = "/home/Neko/Log/"
accounts_conf_path = '/home/Neko/config.yml'
mkpath = "/home/Neko/Log/" + picture_time
# 创建文件夹
mkdir(mkpath)
# 保存Log日志文件至txt
sys.stdout = Logger(path + picture_time + '/Log.txt')  # 保存Log
# 读取用户信息，并向字典中添加Status变量
accounts = Neko_add.get_yaml(accounts_conf_path)
...
for each in accounts:
    if not check_enable(each):
        continue
    try:
        each_driver = webdriver.Chrome(chrome_options=chrome_options)
...
print('——————————————————————————————————————————')
os.system('zip -r /home/Neko/info.zip /home/Neko/Log/*')
print('压缩为info.zip...')
Neko_add.send_result_to_admin(accounts)
print('日志发送给管理员')
# 退出浏览器
